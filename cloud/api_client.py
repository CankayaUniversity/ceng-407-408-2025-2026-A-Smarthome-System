...
def send_sensor_readings_batch(readings: list[dict]) -> dict | None:
    ...
    if not valid:
        logger.warning("send_sensor_readings_batch: no valid readings, skipping")
        return None
...
def send_camera_event(
    ...
    try:
        ...
        if resp.status_code == 201:
            ...
            logger.warning("Camera event failed: %d %s", resp.status_code, resp.text[:200])
            return None
...
def _post_json(path: str, payload: dict) -> dict | None:
    """Generic JSON POST. Returns parsed response or None."""
    url = _url(path)
    try:
        resp = requests.post(
            url,
